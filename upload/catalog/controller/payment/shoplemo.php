...
class ControllerPaymentShoplemo extends Controller
{
    public function index()
    {
        ...
        $this->data = $data;
        $this->template = $this->config->get('config_template') . '/template/payment/shoplemo.tpl';
    ...
    public function get_payment_page_url()
    {
        ...
        $requestBody = [
            ...
            //'callback_url' => 'http://eu.ngrok.io/opencart_15/index.php?route=payment/shoplemo/callback',
    ...
    public function callback()
    {
        ...
        if ($getOrder)
        {
            if ($_data['payment']['payment_status'] == 'COMPLETED' && $getOrder['order_status_id'] == 0)
            {
                ...
                $this->model_checkout_order->confirm($order_id, $this->config->get('shoplemo_order_completed_id'), $note, true);
            }
            elseif ($_data['payment']['payment_status'] == 'FAILED')
            {
                ...
                $this->model_checkout_order->confirm($order_id, $this->config->get('shoplemo_order_canceled_id'), $note, true);
            }
